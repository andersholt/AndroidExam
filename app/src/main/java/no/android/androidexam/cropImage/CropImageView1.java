...
import no.android.androidexam.cropImage.util.AspectRatioUtil;
import no.android.androidexam.cropImage.util.HandleUtil;
import no.android.androidexam.cropImage.util.PaintUtil;
...
public class CropImageView1 extends ImageView {

    // Private Constants ///////////////////////////////////////////////////////////////////////////

    @SuppressWarnings("unused")
    private static final String TAG = CropImageView1.class.getName();
    ...
    private Fragment1.CustomOnClickListener customOnClickListener;
    private Fragment1.CustomOnBoundingBoxChangedListener customOnBoundingBoxChangedListener;
    private float[] actionDown = {-1, -1};
    private float[] actionUp = {-1, -1};
    ...
    public CropImageView1(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        mGuidelinesMode = 1;//typedArray.getInteger(R.styleable.CropImageView_guidelines, 1);
        mFixAspectRatio = false;//typedArray.getBoolean(R.styleable.CropImageView_fixAspectRatio, false);
        mAspectRatioX = 1;//typedArray.getInteger(R.styleable.CropImageView_aspectRatioX, 1);
        mAspectRatioY = 1;//typedArray.getInteger(R.styleable.CropImageView_aspectRatioY, 1);
        // typedArray.recycle();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                this.actionDown = new float[] {event.getX(), event.getY()};
                onActionDown(event.getX(), event.getY());
                return true;

            case MotionEvent.ACTION_UP:
                this.actionUp = new float[]{event.getX(), event.getY()};

                if (this.actionDown[0] == this.actionUp[0] && this.actionDown[1] == this.actionUp[1])
                    customOnClickListener.onClick(null);
                else
                    Log.i("Lecture5", "more then");
                return true;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                customOnBoundingBoxChangedListener.onChanged(getCroppedRect());
    ...
    private void onActionMove(float x, float y) {
        ...
        if (mFixAspectRatio) {
            mPressedHandle.updateCropWindow(x, y, getTargetAspectRatio(), rec, mSnapRadius);
        } else {
            mPressedHandle.updateCropWindow(x, y, rec, mSnapRadius);
        }
        invalidate();
    }


    public void setListeners(@NotNull Fragment1.CustomOnClickListener customOnClickListener, Fragment1.CustomOnBoundingBoxChangedListener customOnBoundingBoxChangedListener) {
        this.customOnClickListener = customOnClickListener;
        this. customOnBoundingBoxChangedListener = customOnBoundingBoxChangedListener;
    }
}